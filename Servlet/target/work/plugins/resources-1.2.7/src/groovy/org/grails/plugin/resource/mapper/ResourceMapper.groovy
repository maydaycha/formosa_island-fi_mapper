...
/**
 * The artefact facade used by the service to communicate with resource test artefacts.
 *
...
class ResourceMapper {
    ...
    @Lazy phase = {
        try {
            ...
            throw new IllegalArgumentException("Resource test ${name} must have a phase property defined")
        }
    }()
    ...
    /**
     * @param artefact an instance of the resource test artefact
    ...
    ResourceMapper(artefact, mappersConfig) {
        ...
        log = LoggerFactory.getLogger('org.grails.plugin.resource.test.' + getName())
    ...
    boolean invokeIfNotExcluded(ResourceMeta resource) {
        ...
        if (excludingPattern) {
            ...
            if (log.debugEnabled) {
                log.debug "Skipping ${resource.sourceUrl} due to definition excluding test"
            }
    ...
    private invoke(ResourceMeta resource) {
        ...
        try {
            ...
            if (artefact.class == e.type && e.method == "map") {
                throw new Exception("The resource test '$name' does not implement the appropriate map method")
            } else {
                throw e
            }
