...
/**
 * This creates the ResourceMapper facades for each resource test artefact and puts them into the bean context
 *
...
class ResourceMappersFactory {
    ...
    static List<ResourceMapper> createResourceMappers(grailsApplication, mappersConfig) {
        ...
        def log = LoggerFactory.getLogger('org.grails.plugin.resource.test.ResourceMappersFactory')
        ...
        // Create an application context to allow the test instances to be autowired
        def beanNames = []
        ...
        mapperOrdering.each { m ->
            if (m.name in operations) {
                throw new IllegalArgumentException(
                    "The test ${m.name} is not valid because there is an operation with the same name. Please change the name of the test.")
            }
        }
