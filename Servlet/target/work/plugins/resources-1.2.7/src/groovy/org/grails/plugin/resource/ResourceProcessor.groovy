...
class ResourceProcessor implements InitializingBean {
    ...
    void processModernResource(request, response) {
        ...
        if (inf?.exists()) {
            ...
            try {
                ...
                // Here we need to let the test add headers etc
    ...
    void applyMappers(ResourceMeta r) {
        ...
        for (m in resourceMappers) {
            if (isMapperEnabledInGlobalConfig(m)) {
                ...
                log.debug "Running test ${m.name} (${m.artefact})"

                if (m.phase != phase) {
                    phase = m.phase
                    log.debug "Entering test phase ${phase}"
                }

                log.debug "Applying test ${m.name} to ${r.processedFile} - delegating? ${r.delegating}"
                long startTime = System.currentTimeMillis()

                boolean appliedMapper = m.invokeIfNotExcluded(r)
                log.debug "Applied test ${m.name} to ${r.processedFile}"
    ...
    /**
     * Test if test is enabled in global configuration.
     *
     * @param test to check
    ...
    /* @todo add this later when we understand what less-css-resources needs
        ...
        // LESS test would make synthetic resources too, and these might also delegate
    ...
    */
    ...
    synchronized reloadMappers() {
        reloading = true
        try {
            log.info("Performing a resource test reload")
            ...
            log.info("Finished resource test reload")
        } finally {
            reloading = false
        }
    }
